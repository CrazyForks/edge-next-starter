...
欢迎使用 Next.js + Cloudflare Workers 全栈模板！按照以下步骤快速启动项目。
...
开发服务器运行在 Workers 运行时环境中，完整支持 Cloudflare 绑定（D1、R2、KV）。
...
确保使用 `pnpm dev`，该命令运行在 Workers 运行时环境中，完整支持 D1/R2/KV。
...
1. 使用 `pnpm dev` 进行开发（运行在 Workers 运行时环境中，完整支持 Cloudflare 绑定）
2. 提交前运行 `pnpm test` 确保测试通过
3. 代码会自动格式化和检查
